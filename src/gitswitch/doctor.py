...
def run_health_check(config_manager, account_manager, git_ops, validation_service) -> bool:
    ...
    try:
        ...
        if config_manager.config_exists():
            ...
            if is_valid:
                ...
                print(f"✅ Configuration is valid with {accounts_count} account(s)")
            else:
    ...
    try:
        ...
        if name and email:
            print(f"✅ Current git config: {name} <{email}>")
        else:
            print("⚠️  No git user configuration set")
