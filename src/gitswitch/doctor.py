...
from .colors import format_status, format_header
...
def run_health_check(config_manager, account_manager, git_ops, validation_service) -> bool:
    ...
    print("╔══════════════════════════════════════════════════════════╗")
    print(f"║                {format_header('Gitswitch Health Check')}                    ║")
    print("╚══════════════════════════════════════════════════════════╝")

    all_healthy = True

    # === SYSTEM REQUIREMENTS CHECK ===
    print(f">> {format_header('Checking System Requirements')}...")
    ...
    if is_valid:
        ...
        for tool, info in system_info.items():
            print(format_status(f"[OK] {tool}: {info['version']}"))
    else:
    ...
    # === CONFIGURATION CHECK ===
    print(f"\n>> {format_header('Checking Configuration File')}...")
    print(SEPARATOR_MEDIUM)
    ...
    try:
        ...
        print(format_status(f"[ERROR] Error checking configuration: {e}"))
        all_healthy = False

    # === ACCOUNTS CHECK ===
    print(f"\n>> {format_header('Checking Accounts')}...")
    print(SEPARATOR_MEDIUM)
    ...
    print(f"\n>> {format_header('Testing Git Functionality')}...")
    print(SEPARATOR_MEDIUM)

    try:
        repo_info = git_ops.get_repository_info()
        if repo_info.get("is_repo"):
            print(format_status(f"[OK] Git repository detected"))
    ...
    print("\n" + SEPARATOR_LONG)
    if all_healthy:
        print(format_status("[SUCCESS] All checks passed! Gitswitch is healthy!"))
    else:
